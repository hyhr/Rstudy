package com.r.study.tcp.gateway.utils;
...
public class NetUtils {
    ...
    public static int getAvailablePort(String host, int port) {
        ...
        for (int i = port; i <= MAX_PORT; i++) {
            ...
            try {
                ...
                log.debug("ip:{} port:{" + host + "} is available" + i);
                return i;
            } catch (IOException e) {
                // continue
                log.warn("Can't bind to address [{" + host + "}:{" + i + "}], " +
                        "Maybe 1) The port has been bound. " +
                        "2) The network card of this host is not exists or disable. " +
                        "3) The host is wrong.");
                log.info("Begin try next port(auto +1):{" + i + 1 + "}");
            } finally {
                if (ss != null) {
                    try {
                        ss.close();
    ...
    public static InetAddress getLocalAddress() {
        ...
        try {
            ...
            log.warn("Error when retriving ip address: " + e.getMessage(), e);
        }
        try {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    log.warn("NetUtils getLocalHostBySocket occur Exception!", e);
                }
            }
        } catch (Exception e) {
            log.warn("Can not connect to host {" + remoteAddress.toString() + "}, cause by :{" + e.getMessage() + "}");
        }
        return host;
    }
    ...
    public static List<InetSocketAddress> getIpListByRegistry(String registryIp) {
        ...
        for (int j = 0; j < ips.size(); j++) {
            ...
            try {
                ...
                log.warn("NetUtils getIpListByRegistry occur Exception!", e);
            }
        }
        ...
        if (StringUtils.isNotBlank(whitelist)) {
            ...
            for (String ips : whitelist.replace(',', ';').split(";", -1)) {
                try {
                    ...
                    log.warn("syntax of pattern {" + ips + "} is invalid");
                }
            }
        }
        return false;
    ...
    public static boolean canTelnet(String ip, int port, int timeout) {
        ...
        try {
            ...
            log.warn("NetUtils canTelnet occur Exception!", e);
            return false;
        } finally {
            if (socket != null) {
                try {
